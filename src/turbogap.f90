...
#ifdef _MPIF90
     IF( rank == 0 )THEN
        if (.not. params%do_dd .or. md_istep == 0) then
...
!          Write Hirshfeld volumes unless the user has disabled it
        ...
        do i = 1, n_soap_turbo
           ...
           do j = 1, size(i_beg_list)
              ...
              call get_gap_soap(n_sites, this_n_sites_mpi, n_neigh(this_i_beg:this_i_end), &
        ...
        if( params%do_prediction )then
           ...
           ! send back partial soap forces for the halo regions
